...
namespace EFSBWFActivities
{
    ...
    public class updateItemsByListviewSB
    {
            ...
            using (SPSite site = new SPSite(context.CurrentWebUrl))
            {
                using (SPWeb web = site.OpenWeb())
                {
                    SPWorkflow.CreateHistoryEvent(web, context.WorkflowInstanceId, 0,
                        web.CurrentUser, TimeSpan.Zero, "updateItemsByListviewSB", strMessage, string.Empty);
                }
            }
        }

        public static Hashtable Execute(SPUserCodeWorkflowContext context, string ListViewTitle, string ListTitle, string FieldName, string FieldValue, Boolean NewVersion)
        {
            ...
            try
            {
                using (SPSite site = new SPSite(context.CurrentWebUrl))
                {
                    using (SPWeb objSPWeb = site.OpenWeb())
                    {
                        ...
                        LogDebugInfo(context, string.Format(@"SPListItemCollection.Count={0}, NewVersion={1}", objSPListItemCollection.Count, NewVersion));
